...
export type MailReward =
  | { type: "item"; itemId: string; quantity: number; containedItem?: string; containedQuantity?: number }
...
function normalizeReward(raw: unknown): MailReward | null {
  ...
  if (reward.type === "item" && typeof reward.itemId === "string") {
    ...
    return quantity > 0 ? {
      type: "item", itemId: reward.itemId, quantity,
  ...
  if (reward.type === "copper" || reward.type === "stamina" || reward.type === "life" || reward.type === "growth_points") {
...
function normalizeMailboxState(raw: unknown): MailboxState {
  ...
  return {
    ...
    redeemedCodes: Array.isArray(state.redeemedCodes)
      ? [...new Set(state.redeemedCodes.filter((code): code is string => typeof code === "string"))]
      : [],
  };
}
...
function addRewardItem(bag: PlayerBagData, itemId: string, quantity: number): AddRewardResult {
  const item = createRewardItem(itemId, quantity);
...
export async function claimMailboxMessage(messageId: string): Promise<ClaimMailResult> {
  ...
  const [rawBag, currentCopper, rawStats, rawStamina, rawLife, rawResources] = await Promise.all([
  ...
  for (const reward of message.rewards) {
    ...
    const item = createRewardItem(reward.itemId, reward.quantity, reward.containedItem, reward.containedQuantity);
    ...
    } else result = addRewardItem(nextBag, reward.itemId, reward.quantity);
...
export async function redeemBonusCode(rawCode: string): Promise<RedeemBonusCodeResult> {
  ...
  const messageId = `bonus-code:${code}`;
  const nextState: MailboxState = {
    ...state,
...
export function mailboxRewardLabel(reward: MailReward): string {
  if (reward.type === "item") return `${reward.quantity}× ${ITEM_CATALOG[reward.itemId]?.name ?? reward.itemId}`;
